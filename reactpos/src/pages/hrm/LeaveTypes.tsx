...
const LeaveTypes: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try { setItems(await getLeaveTypes()); } catch { showError('Failed to load leave types'); }
  ...
  const handleSave = async () => {
    if (!form.name) { showError('Leave Type name is required'); return; }
    try {
      if (editingId) await updateLeaveType(editingId, form);
      else await createLeaveType(form);
      setShowModal(false); showSuccess(editingId ? 'Leave Type updated' : 'Leave Type created'); loadData();
    } catch { showError('Failed to save'); }
  };

  const confirmDelete = async () => {
    if (!deleteId) return;
    try { await deleteLeaveType(deleteId); setShowDeleteModal(false); setDeleteId(null); showSuccess('Leave Type deleted'); loadData(); }
  ...
  return (
    <>
      <div className="page-header">
        <div className="add-item d-flex"><div className="page-title"><h4>Leave Types</h4><h6>Manage leave types</h6></div></div>
        <div className="page-btn"><a href="#" className="btn btn-primary" onClick={e => { e.preventDefault(); openAddModal(); }}><i className="ti ti-circle-plus me-1"></i>Add Leave Type</a></div>
      </div>
              ...
              <table className="table datanew">
                <thead>
                  <tr>
                    ...
                    <th>Leave Type</th><th>Leave Quota</th><th>Created On</th><th>Status</th><th className="text-center">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0 ? <tr><td colSpan={6} className="text-center py-4">No leave types found</td></tr> : paginated.map(item => (
                    <tr key={item.id}>
        ...
        <div className="modal fade show d-block" tabIndex={-1} style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header"><h4 className="modal-title">{editingId ? 'Edit Leave Type' : 'Add Leave Type'}</h4><button type="button" className="close" onClick={() => setShowModal(false)}><span>&times;</span></button></div>
              <div className="modal-body">
                <div className="mb-3"><label className="form-label">Leave Type<span className="text-danger ms-1">*</span></label><input type="text" className="form-control" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
                <div className="mb-3"><label className="form-label">Leave Quota</label><input type="number" className="form-control" value={form.quota} onChange={e => setForm({ ...form, quota: parseInt(e.target.value) || 0 })} /></div>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content p-5">
              <div className="modal-body text-center p-0">
                ...
                <h4>Delete Leave Type</h4><p className="text-muted">Are you sure you want to delete this leave type?</p>
