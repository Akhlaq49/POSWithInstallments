...
const Designation: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setItems(desigs);
      setDepartments(deps);
    } catch { showError('Failed to load designations'); }
  ...
  const handleSave = async () => {
    if (!form.name) { showError('Designation name is required'); return; }
    try {
      if (editingId) await updateDesignation(editingId, form);
      else await createDesignation(form);
      setShowModal(false); showSuccess(editingId ? 'Designation updated' : 'Designation created'); loadData();
    } catch { showError('Failed to save'); }
  };

  const confirmDelete = async () => {
    if (!deleteId) return;
    try { await deleteDesignation(deleteId); setShowDeleteModal(false); setDeleteId(null); showSuccess('Designation deleted'); loadData(); }
  ...
  return (
    <>
      <div className="page-header">
        <div className="add-item d-flex"><div className="page-title"><h4>Designation</h4><h6>Manage designations</h6></div></div>
        <div className="page-btn"><a href="#" className="btn btn-primary" onClick={e => { e.preventDefault(); openAddModal(); }}><i className="ti ti-circle-plus me-1"></i>Add Designation</a></div>
      </div>
              ...
              <table className="table datanew">
                <thead>
                  <tr>
                    ...
                    <th>Designation</th><th>Department</th><th>Members</th><th>Created On</th><th>Status</th><th className="text-center">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0 ? <tr><td colSpan={7} className="text-center py-4">No designations found</td></tr> : paginated.map(item => (
                    <tr key={item.id}>
        ...
        <div className="modal fade show d-block" tabIndex={-1} style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header"><h4 className="modal-title">{editingId ? 'Edit Designation' : 'Add Designation'}</h4><button type="button" className="close" onClick={() => setShowModal(false)}><span>&times;</span></button></div>
              <div className="modal-body">
                <div className="mb-3"><label className="form-label">Designation<span className="text-danger ms-1">*</span></label><input type="text" className="form-control" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content p-5">
              <div className="modal-body text-center p-0">
                ...
                <h4>Delete Designation</h4><p className="text-muted">Are you sure you want to delete this designation?</p>
