...
const CustomerDueReport: React.FC = () => {
  ...
  const cols = ['Reference', 'Code', 'Customer', 'Total Amount', 'Paid', 'Due', 'Status'];
  ...
  return (
    <>
      <PageHeader title="Customer Due Report" breadcrumbs={[{ title: 'Reports' }, { title: 'Customer Due Report' }]} />

      <ul className="nav nav-pills mb-3">
        <li className="nav-item"><Link className="nav-link" to="/customer-report">Customer Report</Link></li>
        <li className="nav-item"><Link className="nav-link active" to="/customer-due-report">Customer Due Report</Link></li>
      </ul>
        ...
        <div className="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
          ...
          <div className="d-flex align-items-center gap-2">
            ...
            <ExportButtons onExportExcel={() => exportToExcel(cols, rows, 'customer-due-report')}
              onExportPDF={() => exportToPDF(cols, rows, 'customer-due-report', 'Customer Due Report')} />
          </div>
        </div>
